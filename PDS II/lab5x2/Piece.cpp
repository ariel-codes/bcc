...
Piece::Piece(std::string symbol, std::string currentPosition,
...
void Piece::checkMovement() {
  for (unsigned int i = 0; i < this->movement.size(); i++) {
    if (this->currentPosition.first + this->movement[i].first ==
            this->nextPosition.first &&
        this->currentPosition.second + this->movement[i].second ==
            this->nextPosition.second) {
      std::cout << "Moving from (" << char((this->currentPosition.first + 97))
                << this->currentPosition.second << ") to ("
                << char((this->nextPosition.first + 97))
                << this->nextPosition.second << ")" << std::endl;
      this->currentPosition.first = this->nextPosition.first;
      this->currentPosition.second = this->nextPosition.second;
    }
  }
  std::cout << "I cannot move from ("
            << char((this->currentPosition.first + 97))
            << this->currentPosition.second << ") to ("
            << char((this->nextPosition.first + 97))
            << this->nextPosition.second << ")" << std::endl;
}
...
void Piece::printPossibleMoves() {
  ...
  for (std::size_t i = 0; i < this->movement.size(); i++) {
    int finalLetterCoordinate =
        this->currentPosition.first + this->movement[i].first;
    int finalNumberCoordinate =
        this->currentPosition.second + this->movement[i].second;
    if (finalLetterCoordinate < BOARD_SIZE && finalLetterCoordinate >= 0 &&
        finalNumberCoordinate <= BOARD_SIZE && finalNumberCoordinate > 0) {
      std::string key = char(finalLetterCoordinate + 97) +
                        std::to_string(finalNumberCoordinate);
  ...
  std::cout << "(" << this->name << ")"
            << " Current Position: ("
            << char((this->currentPosition.first + 97))
            << this->currentPosition.second << ")" << std::endl;
  std::cout << "I have " << legalMovements.size() << " legal movements."
            << std::endl;
  for (auto move : this->legalMovements) {
    std::cout << "(" << char(this->currentPosition.first + 97)
              << this->currentPosition.second << ")->(" << move << ")"
              << std::endl;
  }
}

void Knight::setMovements() {
  this->movement = {{1, -2}, {2, -1}, {2, 1},   {1, 2},
                    {-1, 2}, {-2, 1}, {-2, -1}, {-1, -2}};
}
